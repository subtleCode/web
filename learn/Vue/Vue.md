...
局部组件在Vue实例的components中注册，仅在当前Vue实例中注册。

## 父子组件

在组件(父组件)构造器中的components属性注册组件(子组件)，然后在将父组件主注册(全局或局部)。

如果将Vue实例(root)也看作组件，再结合组件的作用域，组件只能作用于父组件之后内。
...
在组件中也有一个data属性，但它不是存储的对象，而是函数，然后由这个函数返回对象(对象中存储数据)。
